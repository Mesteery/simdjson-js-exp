...
const { find_structural_indexes } = new WebAssembly.Instance(
  new WebAssembly.Module(require('fs').readFileSync('./simdjson_ops.wasm')),
).exports;
const { findStructuralIndexes } = require('./binding.js');
...
//const cdata = new TextEncoder().encode(data).buffer;

let bdata = readFileSync('./large-file.json');
const data = bdata.toString();

//const cdata = new TextEncoder().encode(bdata).buffer;
// add tests
suite
  .add('napi rs', function () {
    const indexes = findStructuralIndexes(bdata);
    for (let i = 0; i < indexes.length; i++) indexes[i];
  })
  .add('napi c', function () {
    const indexes = hello(bdata);
    for (let i = 0; i < indexes.length; i++) indexes[i];
  })
  .add('json', function () {
    JSON.parse(data);
